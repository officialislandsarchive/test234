...
local DevTabSelection = Tabs.DeveloperTab:AddSection("Developer Tab")

local vendingTypeFilter = "T1 Vending"
local isPerVending = false
local interactionDelay = 0.01
local maxVendingCoins = 5000000000
...
local function filterVendingMachines(blocks)
    local filteredMachines = {}
    for _, block in blocks:GetChildren() do
        if (block.Name == "vendingMachine" and (vendingTypeFilter == "T1 Vending" or vendingTypeFilter == "Both")) or
           (block.Name == "vendingMachine1" and (vendingTypeFilter == "T2 Vending" or vendingTypeFilter == "Both")) then
            table.insert(filteredMachines, block)
        end
    end
    return filteredMachines
end

local function handleVendingInteraction(blocks, callback)
    local processed = {}
    ...
    for _, block in ipairs(blocks) do
        if not processed[block] then
            ...
            callback(block, netService)
            netService[closeVendingString]:FireServer({ ["vendingMachine"] = block })
            task.wait(interactionDelay)
        end
    end
end

local SpeedSlider = DevTabSelection:AddSlider("SpeedSlider", {
    ...
    Callback = function(Value)
        ...
        print("Interaction Speed set to:", interactionDelay)
    end
})

local VendingTypeDropdown = DevTabSelection:AddDropdown("VendingType", {
    Title = "Vending Machine Type",
    Values = { "T1 Vending", "T2 Vending", "Both" },
    Multi = false,
    Default = 1,
    Callback = function(Value)
        vendingTypeFilter = Value
        print("Vending Machine Type set to:", vendingTypeFilter)
    end
})

local PerVendingMode = DevTabSelection:AddToggle("PerVendingMode", { Title = "Per Vending Mode", Default = false })
PerVendingMode:OnChanged(function()
    local Value = Options.PerVendingMode.Value
    isPerVending = Value
    print("Per Vending Mode:", isPerVending)
end)

local function depositCoins(blocks, totalAmount)
    if isPerVending then
        handleVendingInteraction(blocks, function(block, netService)
            local depositCoinsString = "vgvigLycuBykEisGk/cuvpgbjalvx"
            netService[depositCoinsString]:FireServer(depositCoinsString, {
                [1] = { ["vendingMachine"] = block, ["amount"] = totalAmount }
            })
        end)
    else
        local count = #blocks
        local baseDeposit = math.floor(totalAmount / count)
        local remainder = totalAmount % count
        handleVendingInteraction(blocks, function(block, netService)
            local depositCoinsString = "vgvigLycuBykEisGk/cuvpgbjalvx"
            local amount = baseDeposit
            if remainder > 0 then
                amount = amount + 1
                remainder = remainder - 1
            end
            netService[depositCoinsString]:FireServer(depositCoinsString, {
                [1] = { ["vendingMachine"] = block, ["amount"] = amount }
            })
        end)
    end
end

local function withdrawCoins(blocks, totalAmount)
    ...
    handleVendingInteraction(blocks, function(block, netService)
        ...
        if coinBalance and coinBalance:IsA("IntValue") then
            ...
            local amountToWithdraw = math.min(coinBalance.Value, remainingAmount)
            remainingAmount = remainingAmount - amountToWithdraw
            netService[takeCoinsString]:FireServer(takeCoinsString, {
                [1] = { ["vendingMachine"] = block, ["amount"] = amountToWithdraw }
            })
        end
    end)
end

local DepositCoins = DevTabSelection:AddInput("DepositCoins", {
    Title = "Deposit Coins",
    Placeholder = "Enter coin amount",
    Numeric = true,
    Callback = function(Value)
        local totalAmount = tonumber(Value) or 0
        local closestIsland = getClosestIsland()
        if not closestIsland then
            print("No vending machines found nearby.")
            return
        end

        local blocks = filterVendingMachines(closestIsland.Blocks)
        if #blocks == 0 then
            print("No vending machines found matching the selected type.")
            return
        end

        depositCoins(blocks, totalAmount)
    end
})

local WithdrawCoins = DevTabSelection:AddInput("WithdrawCoins", {
    ...
    Callback = function(Value)
        ...
        withdrawCoins(blocks, totalAmount)
    end
})
    ...
    Callback = function()
        ...
        handleVendingInteraction(blocks, function(block, netService)
            ...
            if coinBalance and coinBalance:IsA("IntValue") then
                local takeCoinsString = "vgvigLycuBykEisGk/thdheckjectsj"
                netService[takeCoinsString]:FireServer(takeCoinsString, {
                    [1] = { ["vendingMachine"] = block, ["amount"] = coinBalance.Value }
                })
            end
        end)
    end
...
local DepositAll = DevTabSelection:AddButton({
    ...
    Callback = function()
        ...
        handleVendingInteraction(blocks, function(block, netService)
            local depositCoinsString = "vgvigLycuBykEisGk/cuvpgbjalvx"
            local coinBalance = block:FindFirstChild("CoinBalance")
            local remainingSpace = maxVendingCoins - (coinBalance and coinBalance.Value or 0)
            local depositAmount = math.min(5000000000, remainingSpace)

            netService[depositCoinsString]:FireServer(depositCoinsString, {
                [1] = { ["vendingMachine"] = block, ["amount"] = depositAmount }
            })
        end)
    end
})
